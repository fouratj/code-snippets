...
function checkHorizonalPerimeter(row) {
  ...
  row.forEach((curr, index, arr) => {
    ...
    if (curr && (!prev && !next)) {
      //lone island with sea on both sides
      perimeter += 2;
    } else if (curr && ((prev && !next) || (!prev && next))) {
      //multiple land with sea on either/or side
      perimeter += 1;
    }
  });
...
function getPartner(rowIndex, colIndex) {
  // checks to see if rowIndex exists first as it sometimes wont if index on original row was first or
  //last index of array which will make it out of bouns.
